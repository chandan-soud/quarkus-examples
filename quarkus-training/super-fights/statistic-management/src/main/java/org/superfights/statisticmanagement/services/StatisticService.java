...
import org.superfights.statisticmanagement.dtos.FightResult;
import org.superfights.statisticmanagement.dtos.Score;
import org.superfights.statisticmanagement.dtos.TeamStats;
...
public class StatisticService {
	...
	private RankingService topWinners = new RankingService(10);
	...
	public Flowable<Double> computeTeamStats(Flowable<FightResult> results) {
		return results.map(fr -> teamStats.add(fr));
	}
	...
	public Flowable<Iterable<Score>> computeTopWinners(Flowable<FightResult> results) {
		return results
				// Create a sub-stream per hero and villain - for each winner, we get a new
				// sub-stream
